...
-- Definition 2.5.1
Definition-2-5-1 : ∀ {a b} {A : Set a} {B : Set b} {x y : A × B}
  → (p : x ≡ y)
  → proj₁ x ≡ proj₁ y × proj₂ x ≡ proj₂ y
Definition-2-5-1 {_} {_} {A} {B} {x} {y} p = J (A × B) D d x y p
  where
...
Theorem-2-6-2 {_} {_} {A} {B} (a , a') (b , b') = Definition-2-5-1 , it-isequiv
  --
  where
    inv : a ≡ b × a' ≡ b' → (a , a') ≡ (b , b')
    inv (fst , snd) = J A D d a b fst a' b' snd
      where
    ...
    it-isequiv : isequiv Definition-2-5-1
    it-isequiv = (inv , {!   !}) , (inv , {!   !})
