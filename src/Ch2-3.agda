...
apd {a} b {A} {x} {y} {P} f p = J A D d x y p P f
  where
    -- the predicate
    D : (x y : A) (p : x ≡ y) → _
    D x y p = (P : A → Set b) (f : (z : A) → P z) → transport b P p (f x) ≡ f y
...
transportconst {a} ℓ {A} {x} {y} {B} p b = J A D d x y p B b
  where
    -- the predicate
    D : (x y : A) (p : x ≡ y) → _
    D x y p = (B : Set ℓ) (b : B) → transport ℓ (const B) p b ≡ b
...
--
-- -- lemma-2-3-9
-- lemma-2-3-9 : {a : Level} (b : Level) {A : Set a} {x y : A}
--   → (P : A → Set (a ⊔ b))
--   → (p : x ≡ y)
--   → P x → P y
-- lemma-2-3-9 {a} b {A} {x} {y} P p = J A D d x y p P
--   -- J A D d x y p P
--   where
--     -- the predicate
